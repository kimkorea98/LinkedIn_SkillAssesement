...
#### Q1. Which Code sample will eventually cause the computer to run out of memory? //i dont know exactly
...
- [x] : //because f1(20,10) is already 1 so dont have to calculate next function
...
int main()
{
    ...
    printf("%s", p + p[3] -p[1]); // p[3] = E, p[1] = A so p + 4
...
- [ ] Choice A
- [x] Choice A 
      Choice B No choice // because there are no break
